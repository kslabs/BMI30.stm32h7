...
/* USER CODE BEGIN PV */
/* Private variables ---------------------------------------------------------*/
// Метка времени последнего принятого SOF (ms)
volatile uint32_t g_usb_last_sof_ms = 0;
// Флаг активного программного отключения (SDIS)
volatile uint8_t g_usb_softdisc_active = 0;
static uint32_t g_usb_last_poll_ms = 0;
/* USER CODE END PV */
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  hpcd_USB_OTG_HS.Init.vbus_sensing_enable = DISABLE; /* VBUS sensing не используется (линия не заведена) */
