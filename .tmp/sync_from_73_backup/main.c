...
ADC_HandleTypeDef hadc3;  // Для датчика температуры (DTS)
...
static volatile uint8_t optic_active_hold_seconds = 3u;
...
#define OPTIC_TX_BURST_PERIOD OPTIC_TX_BURST_PATTERN_LEN
...
static volatile uint32_t uart1_led_off_tick = 0;
static uint8_t uart1_rx_byte = 0;
static volatile uint32_t uart1_rx_count = 0;
static volatile uint32_t uart1_last_rx_ms = 0;
...
static volatile uint8_t optic_last_raw_state = 0u;
static volatile uint8_t optic_debounced_state = 0u;
static volatile uint32_t optic_last_debounce_ms = 0u;
static volatile uint32_t optic_pulse_window_start_ms = 0u;
static volatile uint8_t optic_pulse_window_count = 0u;

static volatile ws2812_pattern_t g_ws2812_test_pattern = WS2812_PATTERN_IDLE_BREATHE;

static inline void uart1_rx_led_pulse(void){
  LED_ON();
  uart1_led_off_tick = HAL_GetTick() + 100;
}
...
uint8_t optic_sensor_set_hold_seconds(uint8_t seconds)
{
  ...
  optic_active_hold_seconds = seconds;
  need_usb_status_refresh = 1u;
  return optic_active_hold_seconds;
}
...
static void optic_sensor_service(uint32_t now_ms)
{
  if ((optic_sensor_state_public != 0u) &&
      ((now_ms - optic_last_pulse_ms) > ((uint32_t)optic_active_hold_seconds * 1000u))) {
...
static const uint32_t build_signature_hex = 0xA5B6C7D8;
...
static const char* reset_cause_str(uint32_t rsr){
  ...
  if(rsr & RCC_RSR_SFTRSTF)   return "SOFT";
...
static volatile uint8_t iwdg_enabled_runtime = 0;
...
static void sync_phase_handle_irq_fast(uint16_t sample_idx, uint16_t active_samples)
{
  ...
  pulse_spacing = tim15_phase_pulse_spacing_buffers(abs_phase);
...
int main(void)
{
  ...
  dr_port->MODER  &= ~(3u << (dr_idx*2));
  dr_port->MODER  |=  (1u << (dr_idx*2));
  ...
  MX_ADC1_Init();
  MX_ADC2_Init();
  MX_ADC3_Init();  // ADC3 для температурного датчика
...
/**
  * @brief ADC3 Initialization Function (DTS - Digital Temperature Sensor)
  * @param None
  * @retval None
  */
static void MX_ADC3_Init(void)
{
  ...
  /** Common config: ADC3 для DTS (опрос по требованию, без внешних триггеров)
  */
  hadc3.Instance = ADC3;
  hadc3.Init.ClockPrescaler = ADC_CLOCK_ASYNC_DIV1;
  hadc3.Init.Resolution = ADC_RESOLUTION_12B;  /* 12-бит хватает для DTS */
  ...
  hadc3.Init.ContinuousConvMode = DISABLE;  /* Одиночная конверсия по запросу */
  hadc3.Init.NbrOfConversion = 1;
  hadc3.Init.DiscontinuousConvMode = DISABLE;
  hadc3.Init.ExternalTrigConv = ADC_SOFTWARE_START;  /* Запуск по SW */
  hadc3.Init.ExternalTrigConvEdge = ADC_EXTERNALTRIGCONVEDGE_NONE;
  hadc3.Init.ConversionDataManagement = ADC_CONVERSIONDATA_DR;  /* Чтение из DR */
#if defined(ADC_VER_V5_V90)
  ...
  /** Configure DTS channel (ADC3_INP17 - internal temperature sensor)
  */
  sConfig.Channel = ADC_CHANNEL_TEMPSENSOR;  /* DTS канал */
  sConfig.Rank = ADC_REGULAR_RANK_1;
#if defined(ADC_VER_V5_V90)
  sConfig.SamplingTime = ADC3_SAMPLETIME_640CYCLES_5;  /* ADC3-specific max sampling time on H72/H73 */
#else
  sConfig.SamplingTime = ADC_SAMPLETIME_387CYCLES_5;  /* Большая задержка выборки для DTS */
#endif
  sConfig.SingleDiff = ADC_SINGLE_ENDED;
  sConfig.OffsetNumber = ADC_OFFSET_NONE;
  sConfig.Offset = 0;
  ...
  /* Калибровка ADC3 для точности DTS */
  if (HAL_ADCEx_Calibration_Start(&hadc3, ADC_CALIB_OFFSET, ADC_SINGLE_ENDED) != HAL_OK) {
    Error_Handler();
  }

  /* USER CODE BEGIN ADC3_Init 2 */
  printf("[ADC3][DTS] Initialized: Resolution=12b, Channel=TEMPSENSOR, SamplingTime=640.5 cycles\r\n");
...
static void MX_GPIO_Init(void)
{
  ...
  /* PD0: вход фотоприёмника, по умолчанию подтянут к 0 и ловит любое изменение уровня */
...
void DrawUSBStatus(void){
  ...
  return;
    ...
    /* Индикатор SYNC рядом со звёздочкой: 2 цифры slave-count/slot + M/S/O.
