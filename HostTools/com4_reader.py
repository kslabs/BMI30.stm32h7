...
По умолчанию использует COM9; можно переопределить первым аргументом.
"""
import sys
import time
import serial
...
def open_port(dev='COM9', baud=115200, timeout=1.0):
...
def main():
    dev = sys.argv[1] if len(sys.argv) > 1 else 'COM9'
    ser = open_port(dev)
    print(f"Opened {dev}")
