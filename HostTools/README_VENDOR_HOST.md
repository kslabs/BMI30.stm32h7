...
- Повторная команда `SET_DC_CONFIG` с той же или новой скоростью не обнуляет накопленное SAR-состояние; она только меняет текущий режим/settle time. Для полной остановки обучения используйте `FREEZE` или `SET_DC_ADAPT 0`.
