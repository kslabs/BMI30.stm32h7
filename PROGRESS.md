...
## 2025-11-07: Тест стабильности Vendor USB + fail-fast улучшения ✅

**Статус:** Базовый поток Vendor USB (Bulk IN EP 0x83) подтверждён как стабильный на интервале >10с; добавлена инфраструктура раннего прерывания (fail-fast) и корректная поддержка `--pairs 0`.

**Основные изменения (host tool):**
- Добавлены параметры: `--fail-max-timeouts`, `--fail-max-pipes`, `--fail-lag-frames` для автоматического остановa теста при деградации.
- Реализовано накопление лага (device TX - host RX) и немедленный ABORT при превышении порога.
- `--pairs 0` теперь означает *неограниченное чтение до истечения временного окна* (раньше приводило к нулевому числу кадров и ложному впечатлению об отсутствии потока).
- Агрегированные сводки (`[HOST_SUM]`) выводятся по настраиваемому интервалу (`--log-interval`, сегодня 10 секунд).

**Наблюдения текущего прогона (пример первой агрегированной строки):**
```
[HOST_SUM] +10.0s A=527 (52.6/s) B=527 (52.6/s) STAT=0 timeouts=0 pipes=0
```
- Поток стартовал после 1 kick (`[HOST][START][KICK] retry 1/2`), затем `[HOST][START][OK]`.
- Отсутствуют таймауты чтения и pipe‑ошибки в первом интервале.
- Скорость ~52 кадра/с на канал при текущих параметрах окон (не финальный целевой показатель, только индикатор стабильности).

**Что дальше:**
1. Длительный (5 мин) прогон до конца окна (или до fail-fast) для окончательного подтверждения стабильности.
2. Параллельный CDC мониторинг для сопоставления devTX счетчиков (следующий этап проверки содержания и непрерывности).
3. Анализ содержимого кадров (проверка корректности семплов) — отдельная фаза, не влияет на текущий вывод о стабильно идущем потоке.

**Коммит (локально + удалённо):**
- Файл изменён: `HostTools/vendor_usb_start_and_read.py` (добавлены fail-fast и unlimited pairs).
- Запись в `PROGRESS.md` отражает новые механизмы и результаты краткого теста.


## 2025-11-08: Устойчивый 5‑минутный прогон (стабилизация старта) ✅

**Итог:** Поток стабилен на интервале 300 секунд. По логам хоста — без таймаутов и pipe‑ошибок, скорости кадров соответствуют профилю 0. Допустимая задержка 2–3 секунды в начале не влияет на общую устойчивость.

**Ключевые строки из `HostTools/host_rx.log`:**
```
[HOST][SUMMARY] elapsed=300.0s A=21381 B=21352 STAT=0 timeouts=0 pipe_errors=0 dev_tot A=21112 B=20807 TxCplt=42194 seq=20807
```

Расшифровка:
- A/B: суммарные принятые кадры по каналам — тысячи кадров без потерь;
- STAT=0: внутри окна дополнительного статуса не запрашивали (использовался ctrl STAT);
- timeouts=0, pipe_errors=0: ошибок нет;
- dev_tot/TxCplt/seq: счётчики устройства согласованы с приёмом на хосте, отставание не растёт.

**Как запускали (VS Code задачи):**
- Короткий смоук: `Host: Stream 60s (bulk STAT)` — окно 60s, STAT через bulk, агрегаты каждые 5s.
- Длинный прогон: `Host: Stream 300s (ctrl STAT)` — окно 300s, STAT через control (EP0), агрегаты каждые 5s.

**Что изменили для устойчивого старта:**
- Разрешили одноразовый STAT в раннем окне pending_init (mid‑stream guard ослаблен только точечно и разово);
- Ослабили «fresh gating» для первой A: если по прошествии >100ms wr_seq ещё не вырос, но кадр READY — отправляем;
- Добавили безопасную фиксацию `cur_samples_per_frame` по `adc_stream_get_active_samples()` и короткий CDC‑лог `INIT_DBG` (для будущей диагностики ранних окон).

**Рекомендованные критерии PASS для будущих прогонов:**
1) `pipe_errors=0`; 2) `timeouts` в окне равен 0 либо редки в самом старте; 3) суммарные `A` и `B` близки; 4) dev_tot ~ hostRX, лаг не накапливается.

**Дальше (опционально):** записывать JSON‑сводку последнего прогона для автоматической проверки в CI.
